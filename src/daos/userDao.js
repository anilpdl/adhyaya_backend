...
export const getUserFiles = async id => {
  const user = await User.where({ id }).fetch({ withRelated: ['files.user'] });
  const files = user.related('files');
  let filesList = files.filter(file => !file.attributes.is_deleted);
  filesList = filesList || [];
  return filesList;
};
...
export const fetchAll = async () => {
  const users = await User.where({ role: 'student' }).fetchAll({
    withRelated: ['files', 'user_avatar']
  });
...
export const getEducation = async id => {
  try {
    const education = await User.forge({ id }).fetch({
      withRelated: ['educationDetails']
    });
